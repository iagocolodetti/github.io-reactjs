...
import { CifraMatematica as cm } from '../../../utils/criptografia';

function CifraMatematica() {
    ...
    function cifrar() {
        if (texto.length > 0 && chave.length > 0) {
            try {
                setResultado(cm.cifrar(texto, chave));
            } catch (error) {
                setResultado(error);
            }
        }
    ...
    function decifrar() {
        if (texto.length > 0 && chave.length > 0) {
            try {
                setResultado(cm.decifrar(texto, chave));
            } catch (error) {
                setResultado(error);
            }
        }
    ...
    return (
        <>
            ...
            <div className="form-inline justify-content-center">
                <div className="form-group text-center">
                    <button className="btn btn-primary btn-fix" onClick={cifrar} disabled={texto.length === 0 || chave.length < 1 || chave.length > 8}>Cifrar</button>
                    <span className="separar"></span>
                    <button className="btn btn-primary btn-fix" onClick={decifrar} disabled={texto.length === 0 || chave.length < 1 || chave.length > 8}>Decifrar</button>
                </div>
            </div>
            <br/>
            <div className="row justify-content-center">
...
export default CifraMatematica;
